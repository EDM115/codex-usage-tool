...
const DEFAULT_THEME: UsageTheme = makeTheme({ name: "codex-dark", bg: FALLBACK_BG, text: FALLBACK_TEXT, accent: FALLBACK_ACCENT, muted: "#aaaab2", panel: "#0b1019", line: "#202735" }, "bundled fallback");

const BUILTIN_CODEX_THEMES: Record<string, ThemeSeed> = {
  dark: { name: "dark", bg: "#050811", text: "#f7f1e8", accent: "#67e8f9", muted: "#a7a7ad", panel: "#0b1019", line: "#202735" },
  light: { name: "light", bg: "#fbfaf7", text: "#1e1e20", accent: "#005f87", muted: "#62626b", panel: "#ffffff", line: "#d9d5ca" },
  dracula: { name: "dracula", bg: "#00040e", text: "#eae7de", accent: "#ffb86c", muted: "#b7b3c4", panel: "#10121f", line: "#343246" },
  gruvbox: { name: "gruvbox", bg: "#1d2021", text: "#ebdbb2", accent: "#fabd2f", muted: "#a89984", panel: "#282828", line: "#504945" },
  monokai: { name: "monokai", bg: "#272822", text: "#f8f8f2", accent: "#a6e22e", muted: "#c1c1b8", panel: "#1f201b", line: "#49483e" },
  nord: { name: "nord", bg: "#2e3440", text: "#eceff4", accent: "#88c0d0", muted: "#d8dee9", panel: "#3b4252", line: "#4c566a" },
  solarized_dark: { name: "solarized_dark", bg: "#002b36", text: "#eee8d5", accent: "#2aa198", muted: "#93a1a1", panel: "#073642", line: "#586e75" },
  solarized_light: { name: "solarized_light", bg: "#fdf6e3", text: "#073642", accent: "#268bd2", muted: "#657b83", panel: "#eee8d5", line: "#93a1a1" },
};
...
export async function resolveUsageTheme(codexHomes: CodexHome[]): Promise<UsageTheme> {
  ...
  if (themeName) {
    ...
    if (builtin) return makeTheme(builtin, `bundled Codex theme backup for ${themeName}`);
  }
...
function firstCodexConfig(codexHomes: CodexHome[]): { path: string; text: string } | null {
  for (const home of codexHomes) {
    for (const filename of ["config.toml", "codex.toml"]) {
      const configPath = path.join(home.path, filename);
...
function themeFromConfig(text: string, configPath: string): UsageTheme | null {
  ...
  if (!accent && !bg && !textColor) return null;
  const semanticSkill = readStringKey(text, "desktop.appearanceDarkChromeTheme.semanticColors", "skill");
  const seed = {
    ...
    muted: blendHex(normalizeHex(textColor) ?? DEFAULT_THEME.colors.text, normalizeHex(bg) ?? DEFAULT_THEME.colors.bg, 0.68),
    panel: blendHex(normalizeHex(textColor) ?? DEFAULT_THEME.colors.text, normalizeHex(bg) ?? DEFAULT_THEME.colors.bg, 0.07),
    line: blendHex(normalizeHex(textColor) ?? DEFAULT_THEME.colors.text, normalizeHex(bg) ?? DEFAULT_THEME.colors.bg, 0.20),
  };
...
async function tryFetchNamedTheme(name: string): Promise<UsageTheme | null> {
  for (const url of REMOTE_THEME_URLS) {
    try {
      const response = await fetch(url, { headers: { "User-Agent": "codex-usage-tool/0.2", Accept: "text/plain" } });
      if (!response.ok) continue;
      const text = await response.text();
      const parsed = parseThemeSeedFromText(name, text);
      if (parsed) return makeTheme(parsed, `openai/codex ${url}`);
    } catch {
      continue;
    }
  }
...
function parseThemeSeedFromText(name: string, text: string): ThemeSeed | null {
  ...
  if (hexes.length < 3 || !text.toLowerCase().includes(normalized.replaceAll("_", ""))) return null;
  return {
    name,
    bg: hexes[0],
...
function makeTheme(seed: ThemeSeed, source: string): UsageTheme {
  ...
  return {
    ...
    colors: {
      ...
      accent2,
      warning: "#ffd49b",
      cells: [blendHex(text, bg, 0.12), blendHex(accent, bg, 0.20), blendHex(accent, bg, 0.35), blendHex(accent, bg, 0.52), blendHex(accent, bg, 0.72), accent],
