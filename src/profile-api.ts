...
export async function loadProfile(options: {
  ...
  if (options.noApi) return { fetched: false, error: "API disabled by --no-api" };
  if (!options.auth) return { fetched: false, error: "No auth.json access token found" };
  ...
  try {
    ...
    return { fetched: true, endpoint, error: error instanceof Error ? error.message : String(error) };
  }
}
...
function profileEndpoint(baseUrl: string): string {
  ...
  if (normalized.includes("/backend-api")) return `${normalized}/wham/profiles/me`;
...
function normalizeProfile(value: any): AccountProfileResponse {
  ...
  const buckets = value?.dailyUsageBuckets ?? value?.daily_usage_buckets ?? stats?.daily_usage_buckets ?? stats?.dailyUsageBuckets ?? null;
  return {
    summary: {
